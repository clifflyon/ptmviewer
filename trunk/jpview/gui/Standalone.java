...
public class Standalone extends JApplet implements ActionListener,
	...
	public void mouseDragged(MouseEvent e) {
		synchronized (mutex) {
			...
			// int ptmWidth = ((PTMCanvasBufferedImage) ptmCanvas).displayWidth;
			// int ptmHeight = ((PTMCanvasBufferedImage)
			// ptmCanvas).displayWidth;

			int __x = (int) Math.round(((double) e.getX()) / getWidth()
					* ptm.getWidth());
			int __y = (int) Math.round(((double) e.getY()) / getHeight()
					* ptm.getHeight());

			int x = Math.min(Math.max(__x, 0), ptm.getWidth());
			int y = Math.min(Math.max(__y, 0), ptm.getHeight());
			mouseX = x;
			mouseY = y;
	...
	public void mouseMoved(java.awt.event.MouseEvent me) {
		int mouseY = me.getY();
		int mouseX = me.getX();
		int idx = ptm.getWidth() * mouseY + mouseX;

		Vec3f normal = ((RGBPTM) ptm).getChannelNormals()[0][idx];
		showStatus(normal.toString());
